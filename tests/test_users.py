...
def test_delete_user_admin_soft_delete() -> None:
    response = client.delete("/api/v1/users/user-2")

    assert response.status_code == 200
    assert response.json()["detail"] == get_message("errors.users.user_deleted")


def test_delete_user_cannot_delete_self() -> None:
    response = client.delete("/api/v1/users/user-1")

    assert response.status_code == 400
    assert response.json()["detail"] == get_message("errors.users.cannot_delete_self")
